...
export const up = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema
    .createIndex('idx_collections_name')
    .on('collections')
    .column('name')
    .execute();
  ...
  await db.schema
    ...
    .addColumn('collection_id', 'uuid', (col) =>
      col.references('collections.id').notNull(),
    )
    .addColumn('name', 'varchar(255)', (col) => col.notNull())
    .addColumn('type', 'varchar(10)', (col) =>
      col.notNull().check(sql`type IN ('newspaper', 'magazine', 'book')`),
    )
  ...
  await db.schema
    .createIndex('idx_sources_collection_id')
    .on('sources')
    .column('collection_id')
    .execute();
  await db.schema
    .createIndex('idx_sources_type')
    .on('sources')
    .column('type')
    .execute();
  await db.schema
    .createIndex('idx_sources_language')
    .on('sources')
    .column('language')
    .execute();
  ...
  await db.schema
    ...
    .addColumn('source_id', 'uuid', (col) =>
      col.references('sources.id').notNull(),
    )
  ...
  await db.schema
    .createIndex('idx_editions_source_id')
    .on('editions')
    .column('source_id')
    .execute();
  await db.schema
    .createIndex('idx_editions_published_at')
    .on('editions')
    .column('published_at')
    .execute();
};
...
export const down = async (db: Kysely<unknown>): Promise<void> => {
  await sql`DROP TRIGGER IF EXISTS set_editions_updated_at ON editions;`.execute(
    db,
  );
  await sql`DROP TRIGGER IF EXISTS set_sources_updated_at ON sources;`.execute(
    db,
  );
  await sql`DROP TRIGGER IF EXISTS set_collections_updated_at ON collections;`.execute(
    db,
  );
